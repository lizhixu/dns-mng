export const en = {
  common: {
    error: 'Error',
    cancel: 'Cancel',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    refresh: 'Refresh',
    actions: 'Actions',
    active: 'Active',
    inactive: 'Inactive',
    top: 'Top',
    backToTop: 'Back to Top',
    hide: 'Hide',
    show: 'Show',
    copy: 'Copy',
    close: 'Close',
    back: 'Back',
    confirmDelete: 'Confirm Delete',
    confirmDeleteMessage: 'Are you sure you want to delete this item? This action cannot be undone.',
    updating: 'Updating...',
    yes: 'Yes',
    no: 'No',
    selected: 'Selected',
    keepCurrentIfBlank: 'Leave blank to keep current {field}',
    fields: {
      apiKey: 'key',
      password: 'password',
    },
    noSearchResults: 'No results match your search',
    previousPage: 'Previous',
    nextPage: 'Next',
    totalItems: 'Total {count}',
    cacheTime: 'Cache Time',
    status: 'Status',
    password: {
      current: 'Current Password',
      new: 'New Password',
      confirm: 'Confirm Password',
      change: 'Change Password',
      mismatch: 'New password and confirm password do not match',
      tooShort: 'New password must be at least 6 characters',
      updated: 'Password updated',
    },
  },

  login: {
    title: 'Login',
    username: 'Username',
    password: ['Pass', 'word'].join(''),
    usernameTooShort: 'Username must be at least 3 characters',
    usernameTooLong: 'Username cannot exceed 50 characters',
    usernameInvalid: 'Username can only contain letters, numbers, underscores and hyphens',
    passwordTooShort: 'Password must be at least 6 characters',
    viewOnGitHub: 'View on GitHub',
    usernamePlaceholder: 'Enter username',
    passwordPlaceholder: 'Enter password',
  },

  layout: {
    title: 'DNS Manager',
    domains: 'Domains',
    logout: 'Logout',
    themeLight: 'Light Mode',
    themeDark: 'Dark Mode',
    themeSystem: 'System Theme',
    settings: 'Settings',
    logsManagement: 'Logs',
    emailNotifications: 'Email Notifications',
    theme: 'Theme Mode',
    language: 'Language',
    dnshe: 'DNSHE',
    messages: 'Messages',
  },

  messages: {
    title: 'Message Center',
    subtitle: 'Renewal reminders and system notifications',
    noMessages: 'No messages',
    markAllRead: 'Mark all as read',
    unread: 'Unread',
    all: 'All',
    domainExpiry: 'Domain Expiry',
    daysRemaining: '{n} days remaining',
    renewNow: 'Renew Now',
    deleted: 'Message deleted',
    markedRead: 'Marked as read',
    markedAllRead: 'All messages marked as read',
  },

  accounts: {
    title: 'Accounts',
    subtitle: 'Manage your DNS provider accounts',
    addAccount: 'Add Account',
    accountName: 'Account Name',
    accountNamePlaceholder: 'e.g. My Personal DNS',
    provider: 'Provider',
    selectProvider: 'Select a provider',
    providerCannotChange: 'Provider cannot be changed',
    apiKey: 'API Key',
    apiKeyPlaceholder: 'Enter your provider API key',
    addedOn: 'Added on',
    viewDomains: 'View Domains',
    editAccount: 'Edit Account',
    deleteAccount: 'Delete Account',
    linkNewAccount: 'Link New Account',
    linkAccount: 'Link Account',
    noAccounts: 'No accounts linked yet',
    linkFirst: 'Link your first account',
    deleteAccountTitle: 'Confirm Delete Account',
    deleteAccountMessage: 'Are you sure you want to delete account "{name}"? This will also delete all domains and records under this account and cannot be undone.',
    tencentcloudFormat: 'Format: SecretId,SecretKey (separated by comma)',
    aliyunFormat: 'Format: AccessKeyId,AccessKeySecret (comma separated; RAM must allow DNS APIs including DescribeDomainNs). Domain list only includes zones whose registrar NS are all delegated to Alibaba Cloud DNS.',
    huaweicloudFormat: 'Format: AccessKeyId,SecretAccessKey; optional 3rd field ProjectId; optional 4th RegionId (e.g. cn-south-1, default cn-north-4). If there are only 3 parts and the 3rd looks like a region (cn-/ap-/…), it is treated as RegionId. Multi-value record sets appear as multiple rows with ids like uuid|index.',
    cloudflareFormat: 'Format: API Token (from Cloudflare Dashboard → My Profile → API Tokens)',
    ndjpFormat: 'Format: Bearer Token (from NDJP NET Dashboard)',
    desecFormat: 'Format: Token (from deSEC Dashboard → Token Management)',
    dnsheFormat: 'Format: API Key,API Secret (comma separated, from DNSHE Dashboard → API Management)',
    ipv64Format: 'Format: API Token (from IPv64.net Dashboard → API Settings)',
    hurricaneFormat: 'Format: username,password (comma separated, from Hurricane Electric login page)',
    vps8Format: 'Format: API Key (from VPS8 Client Area → Profile → API Key)',
    goToProvider: 'Go to Provider Console',
    ddns: {
      label: 'DDNS Token',
      title: 'DDNS Token Management',
      subtitle: 'One token for the entire system, can update all domain records',
      value: 'Token Value',
      enabled: 'Enabled',
      disabled: 'Disabled',
      lastUsed: 'Last Used',
      lastIP: 'Last IP',
      generate: 'Create Token',
      custom: 'Custom Token',
      copy: 'Copy Token',
      copyUrl: 'Copy Update URL',
      confirmDelete: 'Confirm Delete Token',
      confirmDeleteMsg: 'Are you sure you want to delete this DDNS Token? Devices using this token will no longer be able to update DNS records.',
      noTokens: 'No DDNS Tokens',
      createHint: 'Click button to create DDNS token',
      usageExample: 'Usage Example (DuckDNS Compatible)',
    },
  },

  allDomains: {
    title: 'All Domains',
    subtitle: 'View domains across all accounts',
    searchPlaceholder: 'Search domains or accounts...',
    noDomains: 'No domains found',
    loadingFromAccounts: 'Loading domains from all accounts...',
    totalDomains: 'Total',
    renewalUrl: 'Renewal URL',
    renewalModalTitle: 'Edit Renewal Info',
    permanentFree: 'Permanent Free',
    renew: 'Renew',
  },

  dnshe: {
    title: 'DNSHE Domains',
    subtitle: 'Register, delete, renew DNSHE domains and manage DNS resolution ownership',
    selectAccount: 'Select DNSHE account',
    noAccounts: 'No DNSHE accounts. Add one in Accounts first.',
    noAccountSelected: 'Please select a DNSHE account',
    refresh: 'Refresh',
    quota: {
      title: 'Quota',
      used: 'Used',
      base: 'Base',
      inviteBonus: 'Invite Bonus',
      total: 'Total',
      available: 'Available',
      progress: 'Used {used} / {total}',
    },
    autoRenew: {
      title: 'Auto Renew',
      enabled: 'Enable auto renew',
      daysBefore: 'Threshold (days)',
      daysBeforeHint: 'Domains expiring within this many days are renewed automatically',
      lastRun: 'Last run',
      never: 'Never',
      trigger: 'Run now',
      triggering: 'Running...',
      triggerSuccess: 'Done: renewed {renewed}, failed {failed}{details}',
      triggerSuccessDetails: '; renewed: {renewedDomains}; failed: {failedDomains}',
      saved: 'Auto-renew config saved',
    },
    register: {
      button: 'Register Domain',
      title: 'Register Subdomain',
      subdomain: 'Subdomain prefix',
      subdomainPlaceholder: 'e.g. myapp',
      rootdomain: 'Root domain',
      rootdomainPlaceholder: 'e.g. example.com',
      rootdomainCustom: 'Custom...',
      rootdomainSelect: 'Select root domain',
      success: 'Registered: {domain}',
    },
    delete: {
      button: 'Delete',
      confirmTitle: 'Confirm Delete Domain',
      confirmMessage: 'Delete domain {name}? All its DNS records will be removed. This cannot be undone.',
      success: 'Deleted',
    },
    resolution: {
      dnshe: 'DNSHE DNS',
      thirdParty: 'Third-party DNS',
      dnsheHint: 'Uses DNSHE resolution. Appears in All Domains.',
      thirdPartyHint: 'DNS delegated to a third party. Excluded from All Domains (avoids duplicates).',
      toggleToDNSHE: 'Switch to DNSHE DNS',
      toggleToThirdParty: 'Switch to third-party DNS',
      updated: 'Resolution ownership updated',
    },
    resolveToCF: {
      button: 'Resolve to Cloudflare',
      modalTitle: 'Resolve to Cloudflare',
      selectAccount: 'Select Cloudflare account',
      noCFAccounts: 'No Cloudflare accounts found. Please add one in Accounts first.',
      submit: 'Confirm',
      submitting: 'Resolving...',
      success: 'Switched {domain} NS records to Cloudflare (zone: {zone}). Name servers: {ns}',
      hint: 'This will create/reuse a Cloudflare zone and change this domain\'s NS records to Cloudflare-assigned nameservers. The domain will be marked as third-party resolution.',
    },
    domains: {
      title: 'Domains',
      searchPlaceholder: 'Search domains...',
      noDomains: 'No domains for this account',
      loading: 'Loading...',
      manageRecords: 'Manage Records',
      permanentFree: 'Permanent Free',
    },
  },

  domains: {
    title: 'Domains',
    searchPlaceholder: 'Search domains...',
    manageRecords: 'Manage Records',
    noDomains: 'No domains found for this account',
    confirmDeleteTitle: 'Confirm Hide Local Cache',
    confirmDeleteMessage: 'The following local domain cache entries were not found in this refresh result. Confirming will only hide them from local cache and will not delete real domains from the provider.',
    softDeleteNote: 'Note: This is a local cache soft delete. Domains will be automatically restored if they reappear in provider results.',
    restoredDomainsTitle: 'The following domains have been reactivated:',
    accountFallback: 'Account {id}',
  },

  records: {
    title: 'DNS Records',
    addRecord: 'Add Record',
    type: 'Type',
    nodeName: 'Node Name',
    domainName: 'Domain Name',
    searchPlaceholder: 'Search records...',
    allTypes: 'All Types',
    noRecords: 'No records found',
    addDnsRecord: 'Add DNS Record',
    editDnsRecord: 'Edit DNS Record',
    nodeNamePlaceholder: 'e.g. www (or leave empty for root)',
    ttlSeconds: 'TTL (seconds)',
    priority: 'Priority',
    contentLabels: {
      A: 'IPv4 Address',
      AAAA: 'IPv6 Address',
      CNAME: 'Target Host (Alias)',
      MX: 'Mail Server Host',
      TXT: 'Text Content',
      SRV: 'Target Host',
      SPF: 'SPF Content',
      default: 'Content',
    },
    invalidNodeName: 'Invalid node name. Use only letters, numbers, and hyphens. Cannot start or end with a hyphen.',
    rootRecordHint: 'Leave empty or enter @ for root domain record',
    account: 'Account',
    updatedAt: 'Updated At',
    deleteRecordMessage: 'Are you sure you want to delete DNS record "{name}" ({type})? This action cannot be undone.',
    checkDns: 'Check DNS',
    checkDnsTitle: 'DNS Resolution Check Result',
    checkDnsNormal: 'Resolution Normal',
    checkDnsAbnormal: 'Resolution Abnormal',
    checkDnsMatched: 'DNS record matches expected value',
    checkDnsNotMatched: 'DNS record does not match expected value',
    checkDnsFailed: 'DNS query failed',
    checkDnsNoRecords: 'No DNS records found',
    checkDnsFound: 'DNS record found',
    recordType: 'Record Type',
    expectedValue: 'Expected Value',
    actualValue: 'Actual Value',
    noRecordsFound: 'No records found',
    checkTime: 'Check Time',
    dnsServer: 'DNS Server',
  },

  logsManagement: {
    title: 'API Call Logs',
    subtitle: 'View complete records of all API requests and responses',
    schedulerTitle: 'Scheduler Logs',
    schedulerSubtitle: 'View scheduler task status and check results',
    loginTitle: 'Login Logs',
    loginSubtitle: 'View all login attempts and status',
    triggerCheck: 'Run Check',
    operationTab: 'API Calls',
    noOperationLogs: 'No API call logs',
    noSchedulerLogs: 'No scheduler logs',
    noLoginLogs: 'No login logs',
    loginSuccess: 'Login Success',
    loginFailed: 'Login Failed',
    requestHeaders: 'Request Headers',
    requestBody: 'Request Body',
    responseBody: 'Response Body',
    page: 'Page',
    pages: '',
    taskNames: {
      domain_expiry_notification: 'Domain Expiry Notification',
      dnshe_auto_renew: 'DNSHE Auto Renew',
    },
    schedulerDetails: {
      totalDomains: 'Total',
      success: 'Success',
      failed: 'Failed',
      user: 'User',
      domainsUnit: 'domains',
    },
  },

  emailSettings: {
    title: 'Email Notifications',
    subtitle: 'Configure SMTP email service for domain expiry reminders',
    enabled: 'Enable email notifications',
    smtpServer: 'SMTP Server',
    port: 'Port',
    username: 'SMTP Username',
    password: ['SMTP', 'Password'].join(' '),
    fromEmail: 'From Email',
    fromName: 'From Name (optional)',
    toEmail: 'Recipient Email',
    toEmailHint: 'Where expiry reminders will be sent',
    language: 'Email Language',
    languageHint: 'Set the language for email notifications, defaults to Chinese',
    commonSmtp: 'Common SMTP settings:',
    configSaved: 'Email configuration saved',
    testTitle: 'Test email configuration',
    testDesc: 'Send a test email to {email} to verify settings',
    sendTest: 'Send test email',
    testSent: 'Test email sent to {email}, please check inbox',
    providers: {
      gmail: 'Gmail: smtp.gmail.com:587 (requires app password)',
      outlook: 'Outlook: smtp-mail.outlook.com:587',
      qq: 'QQ Mail: smtp.qq.com:587 (requires authorization code)',
      mail163: '163 Mail: smtp.163.com:465',
    },
  },

  expiry: {
    expiredDaysAgo: 'Expired {days} days ago',
    expiresToday: 'Expires today',
    expiresTomorrow: 'Expires tomorrow',
    expiresInDays: 'Expires in {days} days',
    notifyEnabled: 'Enable expiry reminders',
    notifyDaysBefore: 'Notify days before',
    notifyHint: 'Send reminder {days} days before expiry',
  },

  backup: {
    title: 'Backup & Restore',
    subtitle: 'Export or import full configuration (accounts, domain cache, DDNS, email, WHOIS, DNSHE auto-renew, CF Optimize)',
    exportCard: 'Backup (Export)',
    importCard: 'Restore (Import)',
    encryptionPassword: ['Encryption', 'Password (optional)'].join(' '),
    encryptionPasswordPlaceholder: 'Strongly recommended; empty exports plaintext',
    encryptionPasswordHint: 'Encrypted files require this password to restore',
    exportButton: 'Export Backup File',
    exportSuccess: 'Backup file download started',
    exportError: 'Export failed',
    sensitiveWarning: 'The backup file contains sensitive information including API keys, DDNS token, email password, and WHOIS API key. Encryption is recommended.',
    selectFile: 'Select Backup File',
    noFileSelected: 'No file selected',
    restorePasswordPlaceholder: 'Required if backup was encrypted',
    overwrite: 'Overwrite existing configurations',
    overwriteHint: 'When enabled, imported data overwrites matching existing items; when off, only new items are added',
    overwriteConfirm: 'Confirm overwrite import? Existing matching configurations will be replaced with values from the backup file.',
    restoreButton: 'Restore',
    restoreError: 'Restore failed',
    fileTooLarge: 'Backup file is too large. Maximum size is 10MB',
    fileReadError: 'Failed to read backup file. Please choose it again',
    invalidFormat: 'Invalid backup file format',
    encryptedFileHint: 'Encrypted backup detected. Enter the encryption password to restore it.',
    summaryTitle: 'Backup Summary',
    summaryText: 'Accounts: {accounts}; domain cache: {domainCaches}; DDNS: {ddns}; email: {email}; WHOIS: {whois}; DNSHE auto-renew: {dnshe}; CF Optimize: {cfOptimize}.',
    resultTitle: 'Restore Result',
    resultAccounts: 'Accounts',
    resultDomainCaches: 'Domain Cache (incl. renewal and expiry alerts)',
    resultEmailConfig: 'Email Config',
    resultWHOISConfig: 'WHOIS Config',
    resultDNSHEAutoRenew: 'DNSHE Auto Renew',
    resultCFOptimize: 'CF Optimize Config',
    imported: 'Imported',
    skipped: 'Skipped',
  },
  cfOptimize: {
    title: 'CF Optimize',
    subtitle: 'One-click Cloudflare CDN optimization — auto-configure origin records, optimized CNAME, and custom hostnames',
    createButton: 'One-Click Optimize',
    editTitle: 'Edit Optimization Config',
    form: {
      account: 'Cloudflare Account',
      accountPlaceholder: 'Select a Cloudflare account',
      noCFAccount: 'No Cloudflare accounts found. Please add one in Account Management first.',
      permissionHint: 'Note: API Token needs "SSL and Certificates" edit permission, and the account must have Custom Hostnames (Cloudflare for SaaS) enabled.',
      zone: 'Domain',
      zoneLoading: 'Loading domains...',
      zonePlaceholder: 'Select a domain',
      hostname: 'Hostname',
      hostnamePlaceholder: 'e.g. www',
      hostnameHint: 'Subdomain prefix to optimize. Enter "www" for www.example.com',
      originIP: 'Origin IP',
      originIPPlaceholder: 'e.g. 1.2.3.4',
      cnameTarget: 'Optimized CNAME',
      cnameTargetPlaceholder: 'Custom optimized domain',
      cnameCustom: 'Custom...',
      intermediatePrefix: 'Intermediate Prefix',
      intermediatePrefixHint: 'A <prefix>.<domain> CNAME record will be created under your domain to connect the optimized target and your custom hostname (defaults to saas).',
    },
    cnamePresets: {
      cloudflare: 'Asia-Pacific optimized, best for page acceleration',
      cf: 'Dual-stack IPv4/IPv6, general purpose',
      dl: 'Download optimized, stable US-bound nodes',
    },
    table: {
      customHostname: 'Custom Hostname',
      zone: 'Domain',
      originIP: 'Origin IP',
      originDomain: 'Origin Domain',
      intermediateRecord: 'Intermediate Record',
      pointsTo: 'pointing to',
      cnameTarget: 'Optimized Domain',
      status: 'Status',
      sslStatus: 'SSL Status',
      actions: 'Actions',
    },
    status: {
      pending: 'Pending',
      active: 'Active',
      initial: 'Initializing',
      error: 'Error',
    },
    sslStatus: {
      pending: 'Pending',
      active: 'Issued',
      initial: 'Initializing',
      error: 'Error',
    },
    actions: {
      refresh: 'Refresh Status',
      edit: 'Edit',
      delete: 'Delete',
      deleteWithCleanup: 'Delete & Cleanup Records',
    },
    messages: {
      createSuccess: 'Optimization config created successfully',
      createError: 'Creation failed',
      updateSuccess: 'Optimization config updated successfully',
      refreshSuccess: 'Status updated',
      refreshError: 'Refresh failed',
      deleteSuccess: 'Deleted',
      deleteError: 'Delete failed',
      confirmDelete: 'Delete this optimization config?',
      confirmDeleteCleanup: 'Also clean up DNS records and custom hostname on Cloudflare?',
    },
    empty: 'No optimization configs yet',
  },

  whois: {
    title: 'WHOIS Lookup',
    subtitle: 'Look up domain registration info (registrar, dates, nameservers, etc.)',
    placeholder: 'Enter a domain, e.g. example.com',
    button: 'Lookup',
    queryFailed: 'Lookup failed, please try again later',
    keyMissing: 'Please configure the WHOIS API Key first',
    registered: 'Registered',
    unregistered: 'Unregistered or privacy-protected',
    domain: 'Domain',
    created: 'Created',
    changed: 'Updated',
    expires: 'Expires',
    registrar: 'Registrar',
    registrarName: 'Name',
    registrarUrl: 'URL',
    registrarEmail: 'Email',
    registrarPhone: 'Phone',
    nameservers: 'Nameservers',
    status: 'Status',
    dnssec: 'DNSSEC',
    ips: 'Resolved IPs',
    whoisServer: 'WHOIS Server',
    contacts: 'Contacts',
    owner: 'Owner',
    admin: 'Admin',
    tech: 'Tech',
    noContact: 'No contact info',
    showRaw: 'Show raw JSON',
    hideRaw: 'Hide raw JSON',
    configHint: 'Configure your WhoisJSON.com API key for WHOIS lookups. Stored on the server; you can view and edit it here.',
    configApiKey: 'API Key',
    configKeyPlaceholder: 'Enter your WhoisJSON.com API key',
    configEditHint: 'Leave blank to keep current; enter a new key to update it',
    previewKey: 'Show API key',
    hideKey: 'Hide API key',
    configSave: 'Save',
    configSaved: 'WHOIS configuration saved',
    configExpand: 'API Key settings',
  },
};

export default en;
