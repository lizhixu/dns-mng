...
- 前端入口在顶部导航栏右上角铃铛图标（带外边框与未读徽标，点击以弹窗展示，不占用左侧菜单栏）。
...
- **严禁使用 Emoji 表情**：代码、UI 界面文本、日志输出、邮件模板等任何地方均不得出现 Emoji 表情符号（如 📊, ℹ️, 📋, 📧, 👤, ⚠, ⏱, 🔔, ✅ 等）。状态指示一律使用纯文本（如 `[OK]`, `[FAIL]`, `[WARN]`）或 Lucide 等 SVG 图标组件，保持界面风格专业严谨。

### Git 操作约定

- **严禁主动提交 Git**：除非用户在对话中明确发出提交指令（如明确说“提交”、“commit”），否则 Agent 绝对不得主动执行 `git commit` 或 `git push` 操作，所有代码变更默认保留在本地工作区等待用户指示。

### 代码审查（Code Review）要求

完成功能开发或变更后，交付给用户前必须执行 Code Review，重点自查以下维度：
1. **完整性与联调检查**：确认前后端字段名、路由路径、接口参数完全对应，无遗漏的未闭合逻辑或硬编码。
2. **文本与多语言规范**：
   - 全面排查并严禁任何 Emoji 表情符号，状态指示一律使用标准文本（如 `[OK]`, `[FAIL]`）或 Lucide SVG 图标；
   - 中英文国际化（`frontend/src/locales/zh.js` & `en.js`）必须同步新增或更新，不可遗留英文缺失或未汉化文案；
   - 避免在国际化文件中出现易被安全扫描器误报的明文字段（如将 UI 标签名称误判为硬编码凭据）。
3. **主题与自适应体验**：
   - 检查新组件在亮色模式（Light Mode）与暗色模式（Dark Mode）下的配色表现，严禁出现背景缺失导致白底白字或黑底黑字；
   - 确保在移动端（Mobile）视口下能正常显示与交互，弹窗及列表不发生越界或溢出。
4. **编译与运行保障**：
   - 后端必须通过 `go build ./...` 与 `go vet ./...`，无语法错误与未使用的引用；
   - 前端必须通过 `npm run build` 打包构建，无 TypeScript/JSX 语法与打包警告。
5. **安全与敏感数据**：严格排查日志输出与错误返回，确保 API Key、密码、Token、备份内容等绝不落盘日志、不暴露给未授权端点。
